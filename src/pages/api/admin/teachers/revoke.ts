...
import { revokeTeacher, teacherWriteStatus } from '../../../../server/institution-teachers';
...
export const POST: APIRoute = async ({ request, locals, redirect }) => {
  ...
  if (clientTenant && String(clientTenant) !== auth.tenantId) {
    return Response.json({ error: 'No puede revocar docentes de otra institución.', code: 'forbidden' }, { status: 403 });
  }

  const result = await revokeTeacher(user, {
  ...
  if (wantsRedirect) {
    return redirect('/admin/usuarios?ok=revoked', 303);
  }
